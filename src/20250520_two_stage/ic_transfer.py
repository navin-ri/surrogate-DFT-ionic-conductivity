...
# === Step 5: Define transfer model ===
class ICTransferModel(nn.Module):
...
model = ICTransferModel(input_dim=cif_feat.shape[1])
...
with torch.no_grad():
    model.shared[0].weight.copy_(state_dict['net.0.weight'])
    model.shared[0].bias.copy_(state_dict['net.0.bias'])
    model.shared[2].weight.copy_(state_dict['net.2.weight'])
    model.shared[2].bias.copy_(state_dict['net.2.bias'])

# Unfreeze all layers for fine-tuning
for param in model.parameters():
    param.requires_grad = True

# Set new optimizer for all parameters
loss_fn = nn.MSELoss()
